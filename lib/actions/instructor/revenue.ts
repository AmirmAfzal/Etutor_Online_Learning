...
export async function fetchInstructorRevenue(instructorId: string, month: number, year: number) {
  return await getInstructorDailyIncome(instructorId, month, year);
}

export async function fetchCourseRevenue(courseId: string, month: number, year: number) {
